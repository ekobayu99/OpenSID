...
<body>
	...
	<table align="right" style="padding: 5px 20px; border: solid 1px black;">
		<tr>
			<td><strong><?= $log_surat_detil['kode_format'] ?></strong></td>
		</tr>
	</table>
	<table style="margin-top: 10px;" class="disdukcapil">
		...
		<tr>
			...
			<td>:</td>
			<td colspan=6 class="kotak"><?= $data['alamat_sekarang']; ?></td>
		</tr>
	</table>
	<p style="text-align: center;">
		<strong style="font-size: 12pt;">FORMULIR PERMOHONAN PINDAH WNI</strong><br>
		<?= $log_surat_detil['judul_format'] ?><br>
		No. <?= $log_surat_detil['nomor'] ?>
	</p>
	...
	<table class="disdukcapil">
		...
		<tr>
			...
			<td colspan=22 class="kotak"><?= $data_kk['nama_kepala_kk']; ?></td>
		</tr>
		<tr>
			<td>3.</td>
			<td>Alamat</td>
		...
		<tr>
			...
			<?php for ($i = 0; $i < 10; $i++) : ?>
				<td class="kotak satu">
					<?php if (isset($log_surat_detil['telepon'][$i])) : ?>
						<?= trim($log_surat_detil['telepon'][$i]); ?>
					<?php else : ?>
						&nbsp;
					<?php endif; ?>
				</td>
		...
		<tr>
			...
			<td rowspan="2" class="kotak satu"><?= trim($log_surat_detil['alasan_pindah_id'], "'"); ?></td>
			<td rowspan="2">&nbsp;</td>
		...
		<tr>
			...
			<td colspan=8 class="padat">
				<?php if ($log_surat_detil['sebut_alasan']) : ?>
					<span style='text-decoration: none; border-bottom: 1px dotted black;'><?= $log_surat_detil['sebut_alasan']; ?></span>
				<?php else : ?>
					..............................
				<?php endif; ?>
			</td>
		...
		<tr>
			...
			<td colspan=12 class="kotak"><?= $log_surat_detil['alamat_tujuan']; ?></td>
			<td colspan=2 style="text-align: center;">RT</td>
			<?php for ($i = 0; $i < 3; $i++) : ?>
				<td class="kotak satu">
					<?php if (isset($log_surat_detil['rt_tujuan'][$i])) : ?>
						<?= $log_surat_detil['rt_tujuan'][$i]; ?>
					<?php else : ?>
						&nbsp;
					<?php endif; ?>
				</td>
			...
			<?php for ($i = 0; $i < 3; $i++) : ?>
				<td class="kotak satu">
					<?php if (isset($log_surat_detil['rw_tujuan'][$i])) : ?>
						<?= $log_surat_detil['rw_tujuan'][$i]; ?>
					<?php else : ?>
						&nbsp;
					<?php endif; ?>
				</td>
		...
		<tr>
			<td colspan=2>&nbsp;</td>
			<td colspan=7>Dusun/Dukuh/Kampung</td>
			<td colspan=15 class="kotak"><?= $log_surat_detil['dusun_tujuan']; ?></td>
		</tr>

		<tr>
			<td>&nbsp;</td>
			<td>a. Desa/Kelurahan</td>
			<td colspan=8 class="kotak"><?= $log_surat_detil['desa_tujuan']; ?></td>
			<td colspan=4>c. Kab/Kota</td>
			<td colspan=10 class="kotak"><?= $log_surat_detil['kabupaten_tujuan']; ?></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>b. Kecamatan</td>
			<td colspan=8 class="kotak"><?= $log_surat_detil['kecamatan_tujuan']; ?></td>
			<td colspan=4>d. Provinsi</td>
			<td colspan=10 class="kotak"><?= $log_surat_detil['provinsi_tujuan']; ?></td>
		</tr>
		<tr>
			<td colspan=2>&nbsp;</td>
			<td colspan=3>Kode Pos</td>
			<?php for ($i = 0; $i < 5; $i++) : ?>
				<td class="kotak satu">
					<?php if (isset($log_surat_detil['kode_pos_tujuan'][$i])) : ?>
						<?= $log_surat_detil['kode_pos_tujuan'][$i]; ?>
					<?php else : ?>
						&nbsp;
					<?php endif; ?>
				</td>
			...
			<?php for ($i = 0; $i < 10; $i++) : ?>
				<td class="kotak satu">
					<?php if (isset($log_surat_detil['telepon_tujuan'][$i])) : ?>
						<?= trim($log_surat_detil['telepon_tujuan'][$i]); ?>
					<?php else : ?>
						&nbsp;
					<?php endif; ?>
				</td>
		...
		<tr>
			...
			<td rowspan="2" class="kotak satu"><?= trim($log_surat_detil['jenis_kepindahan_id'], "'"); ?></td>
		...
		<tr>
			...
			<td class="kotak satu"><?= ($log_surat_detil['status_kk_tidak_pindah_id']) ? $log_surat_detil['status_kk_tidak_pindah_id'] : "-"; ?></td>
		...
		<tr>
			...
			<td class="kotak satu"><?= $log_surat_detil['status_kk_pindah_id']; ?></td>
			<td colspan=5 class="padat">1. Numpang KK</td>
			<td colspan=6 class="padat">2. Membuat KK Baru</td>
			<td colspan=7 class="padat">3. Nomor KK Tetap</td>
		</tr>
	...
	<table class="pengikut">
		...
		for ($i = 0; $i < MAX_PINDAH; $i++) {
			$nomor = $i + 1;
			$nik_split = str_split($log_surat_detil['pengikut_nik'][$i]);
			...
			echo '<td>' . $log_surat_detil['pengikut_nama'][$i] . '</td>';
			echo '<td>' . $log_surat_detil['pengikut_berlaku_ktp'][$i] . '</td>';
			echo '<td>' . $log_surat_detil['pengikut_status_kawin'][$i] . '</td>';
			echo '</tr>';
		}
	...
	<!-- <table class="ttd">
		...
		<tr>
			<td>( <?= unpenetration($log_surat_detil['pamong']) ?> )</td>
			<td>&nbsp;</td>
			<td>( <?= $log_surat_detil['pengikut_nama'][0] ?> )</td>
		</tr>
	</table>
	<p>
