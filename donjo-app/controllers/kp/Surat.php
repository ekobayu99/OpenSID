...
class Surat extends Admin_Controller {
	...
	public function form($url = '', $clear = '')
	{
		...
		$this->render("surat_kp/form_surat", $data);
	}
	...
	*/
    ...
    public function cek_nik_baru()
    {
        $p = $this->input->post();

        /** 
         * 1. cek ke tweb_penduduk
         * 2, misalkan ada data dengan nik tersebut, maka data diambil dari database
         * 3. misalkan ndak ada baru curl dukcapil
         * 3.1. misalkan ditemukan NIK nya, maka nyimpen ke database
         * 3.2. misal tidak ditemukan, gagal
         */

        $nik = $p['nik'];

        $sampleDate = $p['tgl_lhr'];
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://192.168.65.80:1000/dukcapil/get_json/pmd/call_nik_verify',
            ...
            CURLOPT_POSTFIELDS => $data_to_json,
        ...
        if ($httpcode == 200) {
            ...
            // if (!empty($response))
            $status_nik = false;
            $status_nama_lgkp = false;
            $status_tmpt_lhr = false;
            $status_tgl_lhr = false;
            $status_kab_name = false;
            $status_kec_name = false;
            $status_kel_name = false;

            $jumlah_benar = 0;

            $response_to_array = json_decode($response, true);
            if (!empty($response_to_array['content'][0]['nik'])) {
                if ($response_to_array['content'][0]['nik'] == $p['nik']) {
                    $status_nik = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['nama_lgkp'])) {
                if (substr($response_to_array['content'][0]['nama_lgkp'], 0, 6) == 'Sesuai') {
                    $status_nama_lgkp = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['tmpt_lhr'])) {
                if (substr($response_to_array['content'][0]['tmpt_lhr'], 0, 6) == 'Sesuai') {
                    $status_tmpt_lhr = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['tgl_lhr'])) {
                if (substr($response_to_array['content'][0]['tgl_lhr'], 0, 6) == 'Sesuai') {
                    $status_tgl_lhr = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['kab_name'])) {
                if (substr($response_to_array['content'][0]['kab_name'], 0, 6) == 'Sesuai') {
                    $status_kab_name = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['kec_name'])) {
                if (substr($response_to_array['content'][0]['kec_name'], 0, 6) == 'Sesuai') {
                    $status_kec_name = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['kel_name'])) {
                if (substr($response_to_array['content'][0]['kel_name'], 0, 6) == 'Sesuai') {
                    $status_kel_name = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['no_rt'])) {
                if (substr($response_to_array['content'][0]['no_rt'], 0, 6) == 'Sesuai') {
                    $status_no_rt = true;
                    $jumlah_benar++;
                }
            }
            if (!empty($response_to_array['content'][0]['no_rw'])) {
                if (substr($response_to_array['content'][0]['no_rw'], 0, 6) == 'Sesuai') {
                    $status_no_rw = true;
                    $jumlah_benar++;
                }
            }
            ...
            $balikan = [
                'nama_lgkp' => $status_nama_lgkp,
                'nik' => $status_nik,
                'tmpt_lhr' => $status_tmpt_lhr,
                'tgl_lhr' => $status_tgl_lhr,
                'kab_name' => $status_kab_name,
                'kec_name' => $status_kec_name,
                'kel_name' => $status_kel_name,
                'no_rt' => $status_no_rt,
                'no_rw' => $status_no_rw,
                // 'tmpt_lhr' => true,
            ];
            ...
            // balikkan response dari dukcapil 
            $ret = [
                'success' => true,
                'data' => $balikan,
                'data_asli_dukcapil' => $response_to_array
            ];
    ...
    public function simpan_surat()
    {
        ...
        // nyimpen di table akp_log_surat
