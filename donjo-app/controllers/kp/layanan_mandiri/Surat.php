...
class Surat extends Mandiri_Controller
{
	...
	public function buat($id = '')
	{
		...
		if ($id)
		{
			...
			if (! $permohonan) redirect('layanan-mandiri-kp/surat/buat');
		}
	...
	public function ajax_table_surat_permohonan()
	{
		...
		for ($i=0; $i < count($data); $i++)
		{
			...
			$list_dokumen[$i][] = '<a href="' . site_url('layanan-mandiri-kp/unduh-berkas/' . $data[$i][id]) . '">' . $data[$i][nama] . '</a>';
	...
	public function form($id = '')
	{
		...
		if ($id)
		{
			...
			if (! $permohonan OR ! $post) redirect('layanan-mandiri-kp/surat/buat');
		...
		{
			if (! $post) redirect('layanan-mandiri-kp/surat/buat');
	...
	public function kirim($id = '')
	{
		...
		redirect('layanan-mandiri-kp/permohonan-surat');
	}
	...
	public function download_surat_tte($id)
	{
		$is_mandiri = $this->session->userdata('mandiri');
		$detil_penduduk_layanan_mandiri = $this->session->userdata('is_login');

		$cek_hak_akses_surat = $this->db 
			->where('permohonan_surat.id', $id)
			->where('permohonan_surat.id_pemohon', $detil_penduduk_layanan_mandiri->id_pend)
			->where('akp_surat_tte.is_ttd', 1)
			->join('akp_log_surat_detil', 'permohonan_surat.id = akp_log_surat_detil.id_permohonan_surat')
			->join('akp_surat_tte', 'akp_log_surat_detil.id_log_surat = akp_surat_tte.id_log_surat')
			->select('
				akp_surat_tte.*
			')
			->get('permohonan_surat')->row();
		
		if (!empty($cek_hak_akses_surat)) {
			if (is_file('./desa/upload/dokumen/signed_'.$cek_hak_akses_surat->file_surat)) {
				// $this->load->helper('download');
				// $data   = file_get_contents('./desa/upload/dokumen/signed_' . $cek_hak_akses_surat->file_surat);
				// force_download("Surat_signed_".$id.".pdf", $data);

				$this->output
					->set_content_type('application/pdf')
					->set_output(file_get_contents('./desa/upload/dokumen/signed_' . $cek_hak_akses_surat->file_surat));
			} else {
				show_404();
			}
		} else {
